...
export default function LaunchChecklist() {
  const { lang } = useLang();
  const items = t(lang, 'checklist_items');
    ...
    <div className="bg-card border border-border rounded-xl p-6 card-shadow">
      ...
      {pct === 100 && (
        <div className="mt-5 bg-green-50 border border-green-200 rounded-xl p-4 text-center">
          <p className="text-green-700 font-bold text-sm">{t(lang, 'checklist_done_title')}</p>
          <p className="text-green-600 text-xs mt-1">{t(lang, 'checklist_done_sub')}</p>
        </div>
      )}
    </div>
  );
