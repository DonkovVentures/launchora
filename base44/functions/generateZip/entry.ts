...
const READINESS = (p,n) => {
  ...
  const bar=pct>=95?'████████████████████ 100%':pct>=85?`█████████████████░░░ ${pct}%`:pct>=70?`█████████████░░░░░░░ ${pct}%`:`████████░░░░░░░░░░░░ ${pct}%`;
  const verdict=pct>=95?`🚀 READY TO LAUNCH\nAll ${total} checks passed.`:pct>=85?`⚠️ BETA READY — needs polish before paid launch\n${failed.length} item${failed.length>1?'s':''} require attention.`:pct>=70?`🔍 INTERNAL REVIEW ONLY — not ready for paid customers\n${failed.length} issue${failed.length>1?'s':''} must be resolved.`:`🛑 NOT LAUNCHABLE — ${failed.length} critical items need attention.`;
