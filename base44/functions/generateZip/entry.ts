...
const READINESS = (p,n) => {
  const swc = n.sections.filter(s => s.body && s.body.trim().length > 50);
  const st = n.sections.length;
  const sbText = n.sections.map(s => s.body || '').join(' ');
  const plInSections = hasBannedContent(sbText);
  const ld = n.ma.listing_description || '';
  const lt = n.ma.listing_title || n.title || '';
  const igCaps = Array.isArray(p.social_media_kit?.instagram_captions) ? p.social_media_kit.instagram_captions : [];
  const allText = [n.title,n.subtitle,n.promise,n.audience,ld,lt,sbText,...igCaps,n.launchPlan].join(' ');
  const globalPH = hasBannedContent(allText);
  const phFound = BANNED.filter(b => allText.toLowerCase().includes(b));
  const isTP = isTemplatePack(n);
  const checks = [
    ...
    {cat:'CONTENT',  l:'Content sections — 3+ generated',                      ok:st>=3, fix:`Only ${st} sections. Use Studio → regenerate sections.`},
    ...
    {cat:'CONTENT',  l:'No banned placeholders in sections',                   ok:!plInSections, fix:'Sections contain placeholder text. Review and replace in Studio.'},
    {cat:'TEMPLATES',l:isTP?'Template pack has 3+ assets':'Template check (N/A — not a template pack)', ok:!isTP||st>=3, fix:'Fewer than 3 template assets. Regenerate sections.'},
  ...
  const passed=checks.filter(c=>c.ok), failed=checks.filter(c=>!c.ok);
  const score=passed.length, total=checks.length, pct=Math.round((score/total)*100);
  ...
  const verdict=pct===100?`🚀 PERFECT — READY TO LAUNCH\nAll ${total} checks passed. Follow the 7-Day Launch Plan.`:pct>=85?`✅ READY TO LAUNCH (minor gaps)\n${failed.length} item${failed.length>1?'s':''} flagged. Safe to launch — fix these after launch.`:pct>=65?`⚠️ ALMOST READY — ${total-score} items remaining\nAddress the failed checks above before launching.`:`🛠 NOT READY — DO NOT LAUNCH YET\n${failed.length} critical items need attention. Fix in Launchora Studio before re-exporting.`;
...
const UPSELL = (p,n) => {
  ...
  const uProd=isRE?`Luxury Agent Brand System Vol. 2 — 10 Advanced Presentation Templates`:`${n.title} — Extended Pack (10 Additional Templates)`;
  const uPitch=isRE?`"Vol. 2 adds 10 bespoke templates: expired listing pitch decks, investor portfolio pages, luxury rental brochures, and a full open house signage suite."`:`"Adds 10 more templates for ${n.niche} professionals covering advanced use cases and seasonal campaigns."`;
  const b1=isRE?`Luxury Real Estate Copywriting Swipe File — 50 Proven Listing Description Formulas`:`${n.niche} Copywriting Swipe File — 40 Proven Copy Formulas`;
  const b1p=isRE?`"Beautiful templates sell better with professional copy — 50 luxury listing descriptions ready to adapt for penthouses, estates, and high-rise condos."`:`"40 ready-to-adapt copy formulas for listings, emails, and social posts in ${n.niche}."`;
  const b2=isRE?`Listing Appointment Conversion Toolkit — Seller Scripts, Pre-Listing Package, Objection Handlers`:`${n.niche} Client Conversion Toolkit — Scripts, Intake Forms, Follow-Up Templates`;
  const b2p=isRE?`"Verbal scripts and a full pre-listing PDF to send before every listing appointment — walk in ready to close."`:`"From first contact to closed deal — scripts and documents to convert more leads in ${n.niche}."`;
  const ob=isRE?`Luxury Color & Font Pairing Guide — 5 Editorial Palettes for Real Estate`:`${n.niche} Brand Style Guide — Color, Font, and Layout Rules (1-Page Reference)`;
  const obp=isRE?`"5 tested luxury color systems with hex codes, font pairings, and usage rules — upgrade your templates immediately."`:`"Know exactly which colors, fonts, and spacing rules to apply to every template — no guessing."`;
  const mem=isRE?`Monthly Luxury Real Estate Design Drop — 2 New Templates Every Month`:`Monthly ${n.niche} Design & Copy Drop — 2 New Templates + 1 Swipe File`;
  const memp=isRE?`"Every month: 2 fresh templates, 1 new copywriting formula, and a seasonal marketing calendar."`:`"New ${n.niche}-specific templates and copy resources every month — stay current."`;
  return `UPSELL & BUNDLE IDEAS — ${n.title}\n${'═'.repeat(60)}\nAll prices are suggestions. Adjust based on your audience and platform.\n${'═'.repeat(60)}\n\nIMMEDIATE UPSELL — Show on Thank-You Page (1-click)\n${hr()}\nProduct: ${uProd}\nPrice: $${uPrice}\nPitch: ${uPitch}\nTip: Show on the thank-you/download page — not in a follow-up email. Strike while intent is highest.\n\n${'─'.repeat(60)}\n\nBUNDLE IDEAS\n${hr()}\nBundle 1: "${n.title}" + "${b1}"\n→ Bundle Price: $${bPrice} (saves ~25% vs buying separately)\n→ Pitch: ${b1p}\n→ Best platform: Gumroad (Bundle product type) or Payhip\n\nBundle 2: "${n.title}" + "${b2}"\n→ Bundle Price: $${Math.round(bPrice*1.3)}\n→ Pitch: ${b2p}\n→ Best platform: Payhip or ThriveCart\n\n${'─'.repeat(60)}\n\nORDER BUMP — Checkbox at Checkout\n${hr()}\nProduct: ${ob}\nPrice: $${obPrice} (added to cart at checkout)\nPitch: ${obp}\nTip: Keep order bumps under $${Math.round(base*0.4)}. Must feel like an obvious no-brainer add-on.\n\n${'─'.repeat(60)}\n\nSUBSCRIPTION / MEMBERSHIP\n${hr()}\nProduct: ${mem}\nPrice: $${mPrice}/month\nPitch: ${memp}\nTip: Offer 1 free month to ${n.title} buyers to build the habit before the first charge.\nBest platform: Payhip (membership) | Gumroad (subscription product)\n\n${'─'.repeat(60)}\n\nWHERE TO ADD UPSELLS\n${hr()}\n• Gumroad: Use "Recommended" products panel on product page\n• Payhip: Configure thank-you page redirect with upsell URL\n• ThriveCart / SamCart: Native one-click upsell post-checkout\n• Email: Send upsell offer in a Day 3 follow-up (48hrs after purchase)\n\nGenerated by Launchora | ${new Date().toLocaleDateString()}`;
};

const NEXT_PRODUCTS = (p,n) => {
  const base=Math.max(n.priceMax,n.priceMin,17);
  // All ranges guaranteed ascending
  const t1a_lo=9, t1a_hi=Math.max(17,Math.round(base*0.5));
  const t1b_lo=Math.max(17,Math.round(base*0.5)), t1b_hi=Math.max(t1b_lo+5,Math.round(base*0.7));
  const t1c_lo=Math.max(t1b_hi,Math.round(base*0.6)), t1c_hi=Math.max(t1c_lo+5,Math.round(base*0.9));
  const t2a_lo=Math.round(base*1.2), t2a_hi=Math.round(base*1.8);
  const t2b_lo=Math.round(base*1.5), t2b_hi=Math.round(base*2.2);
  const t3_lo=Math.round(base*2.5), t3_hi=Math.round(base*4);
  const isRE=/real.estate|realt|property|listing|agent/i.test(n.niche+n.title);
  const i1=isRE?`"Luxury Listing Description Swipe File — 30 Proven Formulas for High-End Properties"`:`"${n.niche} Quick-Start Checklist — Launch-Ready in 24 Hours"`;
  const i2=isRE?`"Agent Brand Voice Guide — Tone, Language, and Copy Rules for Luxury Positioning"`:`"${n.keywords[0]||n.niche} Swipe File — 40 Ready-to-Use Copy Formulas"`;
  const i3=isRE?`"Pre-Listing Seller Consultation Package — Scripts, Questions, and Leave-Behind PDF"`:`"${n.niche} 30-Day Action Journal"`;
  const i4=isRE?`"${n.title} Vol. 2 — 10 Advanced Templates for Investor, Rental, and Off-Market Properties"`:`"Advanced ${n.title} — Extended Edition"`;
  const i5=isRE?`"Luxury Agent Content System — 90 Days of Social Posts, Stories, and Email Scripts"`:`"${n.niche} Masterclass Workbook"`;
  const i6=isRE?`"The Complete Luxury Listing System — Templates + Copy + Scripts + Conversion Toolkit"`:`"The Complete ${n.niche} System — Full Bundle"`;
  return `NEXT PRODUCT IDEAS — ${n.title}\n${'═'.repeat(60)}\nBuild your product ecosystem after ${n.title}.\nAll price ranges go from low (launch) to high (mature pricing).\n${'═'.repeat(60)}\n\nTIER 1 — EASY WINS (1–3 days)\n${hr()}\nIDEA 1: ${i1}\n→ Format: PDF / Swipe File | Price: $${t1a_lo}–$${t1a_hi}\n→ Why: Complements your templates. Buyers who want better copy will buy this next.\n\nIDEA 2: ${i2}\n→ Format: PDF Guide | Price: $${t1b_lo}–$${t1b_hi}\n→ Why: Addresses the most common question from ${n.niche} buyers: "What do I actually write?"\n\nIDEA 3: ${i3}\n→ Format: PDF Workbook | Price: $${t1c_lo}–$${t1c_hi}\n→ Why: Gives buyers a system to use alongside your templates over time.\n\nTIER 2 — MEDIUM EFFORT (1–2 weeks)\n${hr()}\nIDEA 4: ${i4}\n→ Format: Template Pack | Price: $${t2a_lo}–$${t2a_hi}\n→ Why: Natural upgrade for buyers who loved the core pack.\n\nIDEA 5: ${i5}\n→ Format: Digital System / Bundle | Price: $${t2b_lo}–$${t2b_hi}\n→ Why: Positions you as the authority for the full ${n.niche} workflow.\n\nTIER 3 — PREMIUM PRODUCT (2–4 weeks)\n${hr()}\nIDEA 6: ${i6}\n→ Format: Premium Bundle | Price: $${t3_lo}–$${t3_hi}\n→ Why: Combines your best products into one definitive offer for serious buyers.\n\nPRODUCT ROADMAP\n${hr()}\nMonth 1: ${n.title} ✅\nMonth 2: Idea 1 (fast add-on, immediate revenue)\nMonth 3: Bundle ${n.title} + Idea 1 for $${Math.round(base*1.4)}\nMonth 4: Idea 4 (Vol. 2 / Advanced version)\nMonth 5: Idea 5 (authority positioning)\nMonth 6+: Idea 6 (full premium system — flagship offer)\n\nUse Launchora to generate any of these next products instantly.\nGenerated by Launchora | ${new Date().toLocaleDateString()}`;
};
