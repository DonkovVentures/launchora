...
const READINESS = (p,n) => {
  ...
  // RULE 10: Only 100% = READY. Any issues = NOT LAUNCHABLE. No partial pass states.
  const bar=failed.length===0?'████████████████████ 100%':`████████░░░░░░░░░░░░ ${pct}% — NOT LAUNCHABLE`;
  const verdict=failed.length===0?`READY TO LAUNCH\nAll ${total} checks passed.`:`NOT LAUNCHABLE — ${failed.length} issue${failed.length>1?'s':''} must be resolved before publishing.\n\nA product with unresolved checks is a work-in-progress, not a launchable product.\nFix all issues below, then re-export.`;
...
Generated by Launchora | launchora.com`;
    ...
    try {
      const qgPayload = {
        ...
        masterGuideMd,
        masterGuideHtml,
        masterGuidePdfSize: masterGuidePdf ? masterGuidePdf.length : 0,
        n: { title:n.title,subtitle:n.subtitle,promise:n.promise,type:n.type,niche:n.niche,platform:n.platform,priceMin:n.priceMin,priceMax:n.priceMax,sections:n.sections,keywords:n.keywords,av:n.av },
        // Pass only string files to quality gate (skip binary PDF)
        files: files.filter(f => typeof f.data === 'string').map(f => ({ name: f.name, data: f.data })),
      };
      const qgRes = await base44.asServiceRole.functions.invoke('exportQualityGate', qgPayload);
      if (qgRes?.data?.ok) {
        qualityGateResult = qgRes.data;
        console.log(`[generateZip] QualityGate: score=${qualityGateResult.qualityScore}% verdict="${qualityGateResult.readinessVerdict}" failed=${qualityGateResult.failedQualityChecks?.length||0}`);
        if (qualityGateResult.failedQualityChecks?.length > 0) {
          for (const fc of qualityGateResult.failedQualityChecks) {
            warnings.push(`[QA] ${fc.file}: ${fc.reason}`);
          }
        }
      } else {
        warnings.push('Quality gate returned non-ok: ' + (qgRes?.data?.error || 'unknown error'));
      }
    } catch(e) {
      warnings.push('Quality gate failed (non-blocking): ' + e.message);
      console.warn('[generateZip] Quality gate error:', e.message);
    }
