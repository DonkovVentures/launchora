...
const ETSY = (p,n) => {
  function buildEtsyTags(n) {
    ...
    let niche_tags;
    if (isRE) niche_tags=['realtor templates','luxury realtor','listing template','real estate kit','property flyer','broker branding','open house flyer','seller pitch deck','agent bio','market report','canva realtor','realty brochure','home listing kit'];
    else if (isFitness) niche_tags=['fitness templates','workout planner','gym program kit','client tracker','fitness branding','training schedule','meal plan template','fitness planner','gym bundle','coach templates','fitness canva','workout bundle','client program'];
    else if (isCoach) niche_tags=['coaching templates','coach branding','client workbook','session notes','onboarding kit','coach canva','intake form','proposal template','coaching bundle','discovery call','client toolkit','coach planner','business coach'];
    else if (isMarketing) niche_tags=['marketing kit','brand templates','content planner','social media kit','canva branding','brand guide','campaign brief','content calendar','marketing bundle','brand toolkit','social templates','caption templates','brand strategy'];
    else niche_tags=[`${ni} templates`,`${ni} planner`,`${ni} kit`,`${ni} bundle`,`${ni} canva`,`${ni} branding`].map(t=>t.slice(0,20));
    const universal=['digital download','canva template','instant download','printable kit','editable template','professional kit','template bundle','done for you','digital template','small biz kit'];
